...
| **A2** | Adaptive Dashboard (the home) | Cards always present (zeros when empty) + a 3-step activation checklist when empty; KPI tiles + a "Latest report" intelligence preview (top finding, biggest risk, #1 action) + "Needs attention" + recent activity when there is data | A1, reporting engine |
| **A3** | Reports hub | Reports becomes the central data/stats hub: a rich card per completed assessment (score, band, top finding, top risk, expected impact, quick actions) + filters; Compare folded in | A1, reporting engine |
| **A4** | Plain-language & naming pass | Retire "Benchmark"/"facility" → "Compare"/"Assessment Target"; sweep every label/button to action-first plain language; consistency across the app | A1–A3 |
