...
Numeric question definitions use `numeric_unit`, `numeric_min`, `numeric_max`, and `numeric_step`; `question_numeric_bands` supplies the immutable scoring map for scored numeric questions. The same configuration is frozen into published content and assessment snapshots.
