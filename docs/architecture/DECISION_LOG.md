...
### DEC-2026-08-04-054: Local Questions Support Seven Plain-Language Answer Formats, Three Optionally Scored

- **Status:** Accepted and implemented.
- **Context:** Workspace-authored local questions (formerly presented as "Tailored by your team") only supported Yes/No and a 1-5 scale, with confusing "reverse the scale" wording, and `/contributions/create` did not match Vytte's polished form system.
- **Decision:** `App\Support\LocalQuestionFormat` defines all seven formats Vytte supports platform-wide (Yes/No, Yes/No/Not applicable, Choose one option, Choose all that apply, 1-5 rating, Number, Written answer) for local questions, each with complete authenticated and public rendering, validation, persistence, and report display. Only Yes/No, Yes/No/Not applicable, and 1-5 rating may optionally contribute to a clearly labelled **optional local score**; an author explicitly marks each eligible question as scored via `is_scored`. The other four formats always remain contextual-only — visible in the report but never scored, since no calibrated scoring rule exists for open-ended or free-choice input. "Reverse the scale" was replaced with plain direction wording ("5 represents the strongest or best result" / "1 represents the strongest or best result"), with `reversed` read as a backward-compatible fallback for pre-existing data.
- **Boundary:** None of this touches a published template's frozen scoring profile or benchmark. `CustomSectionScoringService` continues to compute the optional local score entirely separately from `ScoringService`. `/contributions/create` was rebuilt with the same `x-form-section` / `x-form-field` / `x-text-input` / `x-help-callout` components used elsewhere, but contribution remains a private draft until a publisher assigns scoring and a future published template version — it does not itself create or alter governed content.

### DEC-2026-08-05-055: How Scoring Works Is A Plain-Language Page Open To Every Signed-In User, Not Just Platform Admin

- **Status:** Accepted and implemented.
- **Context:** DEC-2026-08-04-054 introduced the published-score-vs-optional-local-score distinction, but a workspace user had no way to understand it except by reading code or an AGENTS.md-level explanation from support. Confining that explanation to Platform Admin would have hidden it from exactly the audience confused by it — ordinary workspace users authoring local questions.
- **Decision:** `App\Http\Controllers\ScoringModelController` renders `scoring-model.index` behind ordinary `auth` middleware (no `EnsurePlatformAdmin` gate), reading its content live from `App\Support\AnswerFormat` and `App\Support\LocalQuestionFormat` so the page can never drift from the actual scoring rule. It is linked from `RoleNavigation::workspace()`'s Library group and `RoleNavigation::platform()`'s main group, and from the help callouts on the local-questions form and the Contribute Questions form — the two places the confusion actually surfaces.
- **Reason:** Explaining already-public methodology is not sensitive configuration. Restricting it to Platform Admin would optimize for the wrong reader.

### DEC-2026-08-17-056: Every Published Template Is Universally Available During Beta

- **Status:** Accepted, implemented, and covered by a regression test.
...
### DEC-2026-08-18-061: Review Only When Required And Compare Only Like With Like

- **Status:** Accepted and implemented.
- **Response review:** A completed response counts automatically when the frozen template has no respondent eligibility rules. Its decision controls remain available under **Change decision** for exceptional exclusion or test classification, but are not presented as a required step. When eligibility rules exist, submitted responses remain pending, the required review is prominent, and finalisation remains blocked until the configured threshold of reviewed, valid responses is ready.
- **Link language:** Respondent collection explains that its one active link is reusable and that replacement creation appears only after deactivation. Completed Results labels report-sharing actions **Create report link** and **Create another report link**, keeping report access distinct from respondent access.
- **Overall results:** The workspace navigation and page use **Overall Results** instead of Portfolio. Every row, action count, total, and coverage count is restricted to the active workspace. Results are averaged or compared only when their frozen comparison signatures match; unlike instruments remain separate without rank or delta. The obsolete Benchmark UI redirects here so there is one comparison contract.
- **Supersession:** This decision supersedes DEC-2026-07-27-042's plain average, league table, and domain roll-up across unlike targets. Completed assessment and report snapshots remain unchanged.
