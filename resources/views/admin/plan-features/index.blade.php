        ...
        <div class="mb-5 grid gap-4 lg:grid-cols-3">
            @foreach ($plans as $plan)
                <div class="section-card p-5 dark:border-slate-700 dark:bg-slate-800">
                    ...
                    <input name="plans[{{ $plan->plan_code }}][public_label]" value="{{ $plan->public_label }}" class="mt-3 w-full rounded-xl border border-slate-300 bg-white px-3.5 py-2.5 text-sm text-slate-900 shadow-sm focus:border-vytte-500 focus:outline-none focus:ring-2 focus:ring-vytte-500/20 dark:border-slate-600 dark:bg-slate-900 dark:text-white" required>
                    <textarea name="plans[{{ $plan->plan_code }}][description]" rows="2" class="mt-3 w-full rounded-xl border border-slate-300 bg-white px-3.5 py-2.5 text-sm text-slate-900 shadow-sm focus:border-vytte-500 focus:outline-none focus:ring-2 focus:ring-vytte-500/20 dark:border-slate-600 dark:bg-slate-900 dark:text-white">{{ $plan->description }}</textarea>
                    <input name="plans[{{ $plan->plan_code }}][display_order]" type="number" value="{{ $plan->display_order }}" class="mt-3 w-full rounded-xl border border-slate-300 bg-white px-3.5 py-2.5 text-sm text-slate-900 shadow-sm focus:border-vytte-500 focus:outline-none focus:ring-2 focus:ring-vytte-500/20 dark:border-slate-600 dark:bg-slate-900 dark:text-white" required>
                    <textarea name="plans[{{ $plan->plan_code }}][limits_json]" rows="6" class="mt-3 w-full rounded-xl border border-slate-300 bg-white px-3.5 py-2.5 font-mono text-xs text-slate-900 shadow-sm focus:border-vytte-500 focus:outline-none focus:ring-2 focus:ring-vytte-500/20 dark:border-slate-600 dark:bg-slate-900 dark:text-white">{{ json_encode($plan->limits, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) }}</textarea>
