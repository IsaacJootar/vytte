<x-admin-layout title="Respondent preview">
    <div class="mb-5 flex flex-wrap items-start justify-between gap-3">
        <div>
            <a href="{{ route('admin.assessments.quality', $assessment) }}" class="text-sm text-slate-500 hover:underline dark:text-slate-400">&larr; Back to review</a>
    ...
    <div class="mt-5 flex flex-wrap justify-between gap-3">
        <a href="{{ route('admin.assessments.quality', $assessment) }}" class="rounded-xl border border-slate-300 px-5 py-2.5 text-sm font-semibold text-slate-700 dark:border-slate-600 dark:text-slate-200">&larr; Review</a>
        <a href="{{ route('admin.assessments.publish-page', $assessment) }}" class="rounded-xl bg-vytte-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-vytte-700">Continue to publish &rarr;</a>
    </div>
</x-admin-layout>
