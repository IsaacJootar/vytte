...
<div class="page">
        ...
        <div class="header-top">
            ...
            <div class="header-meta">
                @if ($assessment->completed_at)
                    Completed: {{ $assessment->completed_at->format('d M Y') }}<br>
                @endif
                @if ($assessment->assessor_name)
                    Assessor: {{ $assessment->assessor_name }}<br>
                @endif
                Generated: {{ now()->format('d M Y') }}
            </div>
        </div>
        <div class="module-title">{{ $assessment->moduleScope->first()?->module?->module_name ?? 'Assessment' }}</div>
        <div class="target-name">{{ $assessment->target?->name }} · {{ $assessment->project?->name }}</div>
    </div>
    ...
    <div class="score-hero">
        ...
        <div class="score-meta">
            ...
            @if ($score?->maturityLevel)
                <div class="score-maturity">Maturity Level: {{ $score->maturityLevel->level_name }}</div>
            @endif
    ...
    <div class="footer">
        ...
        <span>Assessment ID: {{ substr($assessment->assessment_id, 0, 8) }}...</span>
    </div>
</div>
</body>
</html>
