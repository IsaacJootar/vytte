            ...
            <table class="min-w-full divide-y divide-slate-200 text-sm dark:divide-slate-700">
                <thead class="bg-slate-50 dark:bg-slate-900/50">
                    <tr>
                        ...
                        <th scope="col" class="px-5 py-2.5 text-right text-xs font-semibold text-slate-500 dark:text-slate-400"></th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                    @forelse ($sessions as $session)
                        <tr class="transition-colors {{ $session->submitted_at ? 'cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-700/40' : '' }}"
                            @if ($session->submitted_at) onclick="window.location='{{ route('assessments.respondent-sessions.show', [$assessment, $session]) }}'" @endif>
                            ...
                            <td class="px-5 py-3 text-right">
                                @if ($session->submitted_at)
                                    <span class="text-xs font-semibold text-vytte-700 dark:text-vytte-400">View →</span>
                                @endif
                            </td>
                        </tr>
                    @empty
