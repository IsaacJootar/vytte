<x-app-layout title="Start assessment">
    <div class="max-w-3xl mx-auto">
        ...
        <div class="grid gap-4 sm:grid-cols-2">
            ...
            {{-- Share to others --}}
            <div class="flex flex-col rounded-2xl border p-5 {{ $allowsMultiRespondent ? 'border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800' : 'border-dashed border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-800/50' }}">
                <div class="w-10 h-10 rounded-xl {{ $allowsMultiRespondent ? 'bg-vytte-50 dark:bg-vytte-900/30' : 'bg-slate-100 dark:bg-slate-700' }} flex items-center justify-center mb-3">
                    <svg class="w-5 h-5 {{ $allowsMultiRespondent ? 'text-vytte-600 dark:text-vytte-400' : 'text-slate-400 dark:text-slate-500' }}" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path d="M13 6a3 3 0 11-6 0 3 3 0 016 0zM18 8a2 2 0 11-4 0 2 2 0 014 0zM14 15a4 4 0 00-8 0v3h8v-3zM6 8a2 2 0 11-4 0 2 2 0 014 0zM16 18v-3a5.972 5.972 0 00-.75-2.906A3.005 3.005 0 0119 15v3h-3zM4.75 12.094A5.973 5.973 0 004 15v3H1v-3a3 3 0 013.75-2.906z"/></svg>
                </div>
                <h3 class="text-sm font-bold {{ $allowsMultiRespondent ? 'text-slate-900 dark:text-white' : 'text-slate-500 dark:text-slate-400' }}">Share it for others to answer</h3>
                @if ($allowsMultiRespondent)
                    <p class="mt-1 flex-1 text-xs text-slate-500 dark:text-slate-400">Send a link so several people can answer independently. Their answers are combined and you finalise the result.</p>
                ...
                @else
                    <p class="mt-1 flex-1 text-xs text-slate-400 dark:text-slate-500">This assessment is designed to be answered by one assessor, so it isn't collected from a public link. You can still <span class="font-semibold">share the finished report</span> once it's complete.</p>
                    <span class="mt-4 inline-flex items-center justify-center gap-1.5 rounded-xl border border-slate-200 dark:border-slate-700 px-4 py-2.5 text-sm font-semibold text-slate-400 dark:text-slate-500">Not available for this assessment</span>
                @endif
            </div>
        </div>
    </div>
