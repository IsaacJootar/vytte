<x-app-layout title="Start Assessment">
    ...
    <div class="mb-5">
        <a href="{{ route('projects.show', $project) }}" class="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-700 transition-colors">
    ...
    <div class="mb-6">
        <h1 class="text-xl font-bold text-slate-900 tracking-tight">Start Assessment</h1>
        <p class="mt-0.5 text-sm text-slate-500">Choose a module to assess {{ $project->targets->first()?->name ?? 'this target' }}.</p>
    </div>
    ...
    @else
        <form method="POST" action="{{ route('assessments.store', $project) }}" x-data="{ selected: null }">
            ...
            <div class="space-y-2 mb-6">
                @foreach ($modules as $module)
                    <label class="block cursor-pointer">
                        ...
                        <div class="flex items-start gap-4 p-4 bg-white rounded-xl border transition-all duration-150"
                             :class="selected == '{{ $module->module_id }}'
                                 ? 'border-vytte-500 ring-2 ring-vytte-200 bg-vytte-50'
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-2 mb-0.5">
                                    <span class="text-[10px] font-bold text-vytte-700 uppercase tracking-wide">{{ $module->module_code }}</span>
                                </div>
                                <p class="text-sm font-semibold text-slate-900">{{ $module->module_name }}</p>
            ...
            <button type="submit"
                    :disabled="!selected"
                    x-bind:class="selected ? 'bg-vytte-700 hover:bg-vytte-800 cursor-pointer' : 'bg-slate-200 text-slate-400 cursor-not-allowed'"
