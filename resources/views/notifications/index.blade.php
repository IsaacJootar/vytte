    ...
    <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        @if ($notifications->isEmpty())
            <div class="px-6 py-12 text-center">
                <svg class="mx-auto w-8 h-8 text-slate-300 mb-3" viewBox="0 0 20 20" fill="currentColor">
                ...
                <p class="text-xs text-slate-300 mt-1">You'll be notified when assessments complete or team members join.</p>
            </div>
        @else
                ...
                @foreach ($notifications as $notification)
                    @php
                        ...
                        $iconBg = match ($data['type'] ?? '') {
                            ...
                            'member_joined' => 'bg-vytte-100 text-vytte-700',
                            default => 'bg-slate-100 text-slate-500',
                        };
                    @endphp
                    <div class="flex items-start gap-3.5 px-5 py-4 {{ $isUnread ? 'bg-vytte-50/40' : '' }}">
                        {{-- Type icon --}}
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-start justify-between gap-2">
                                <p class="text-sm font-semibold text-slate-900 {{ $isUnread ? '' : 'text-slate-600' }}">
                                    {{ $data['title'] ?? 'Notification' }}
                                    @if ($isUnread)
                                        <span class="inline-block w-1.5 h-1.5 rounded-full bg-vytte-600 align-middle ml-1 mb-0.5"></span>
                                    @endif
                                </p>
                                <span class="flex-shrink-0 text-[10px] text-slate-400 mt-0.5">
                                    {{ $notification->created_at->diffForHumans() }}
                                </span>
                            </div>
                            <p class="text-xs text-slate-500 mt-0.5">{{ $data['body'] ?? '' }}</p>
                            @if (! empty($data['url']))
