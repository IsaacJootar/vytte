<x-app-layout title="Dashboard">
    ...
    <div class="mb-6 flex items-start justify-between gap-4">
        <div>
            <h1 class="text-xl font-bold text-slate-900 dark:text-white tracking-tight">Dashboard</h1>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-5"
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-5">
            <p class="text-[11px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wide">Active Projects</p>
            <div x-show="!loaded">
                <x-skeleton class="mt-2 h-8 w-16 rounded" />
                <x-skeleton class="mt-2 h-3 w-32 rounded" />
            </div>
            <div x-show="loaded" x-cloak>
                <p class="mt-1.5 text-3xl font-bold text-slate-900 dark:text-white tabular-nums">{{ $activeProjectCount }}</p>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-5">
            <p class="text-[11px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wide">Assessments Run</p>
            <div x-show="!loaded">
                <x-skeleton class="mt-2 h-8 w-16 rounded" />
                <x-skeleton class="mt-2 h-3 w-32 rounded" />
            </div>
            <div x-show="loaded" x-cloak>
                <p class="mt-1.5 text-3xl font-bold text-slate-900 dark:text-white tabular-nums">{{ $totalAssessments }}</p>
                <p class="mt-0.5 text-xs text-slate-400 dark:text-slate-500">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-5">
            <p class="text-[11px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wide">Avg. Score</p>
            <div x-show="!loaded">
                <x-skeleton class="mt-2 h-8 w-16 rounded" />
                <x-skeleton class="mt-2 h-3 w-32 rounded" />
            </div>
            <div x-show="loaded" x-cloak>
                ...
                @else
                    <p class="mt-1.5 text-3xl font-bold text-slate-300 dark:text-slate-600 tabular-nums">—</p>
                    <p class="mt-0.5 text-xs text-slate-400 dark:text-slate-500">No scores yet</p>
                @endif
            </div>
        </div>
    </div>
    ...
    @if ($hasScores)
        <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-5 mb-5"
             x-data="{ loaded: false }" x-init="$nextTick(() => { loaded = true })">
            <p class="text-[11px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wide mb-3">Score Distribution</p>
            ...
            <div x-show="loaded" x-cloak>
                ...
                @endphp
                <div class="flex rounded-full overflow-hidden h-2.5 mb-3 gap-px bg-slate-100 dark:bg-slate-700">
                ...
                @if ($recentProjects->isEmpty())
                    <div class="px-5 py-10 flex flex-col items-center text-center">
                        <div class="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-700 flex items-center justify-center mb-3">
                            <x-heroicon-o-folder class="w-5 h-5 text-slate-400 dark:text-slate-500" />
                        </div>
                        <p class="text-sm font-semibold text-slate-700 dark:text-slate-300">No projects yet</p>
                    ...
                    <div class="divide-y divide-slate-100 dark:divide-slate-700">
                        @foreach ($recentProjects as $project)
                            ...
                            <a href="{{ route('projects.show', $project) }}"
                               class="flex items-center gap-3 px-5 py-3.5 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                                ...
                                <svg class="w-4 h-4 text-slate-300 dark:text-slate-600 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden"
            ...
            <div x-show="loaded" x-cloak>
                @if ($recentAssessments->isEmpty())
                    <div class="px-5 py-10 flex flex-col items-center text-center">
                        <div class="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-700 flex items-center justify-center mb-3">
                            <x-heroicon-o-clipboard-document-list class="w-5 h-5 text-slate-400 dark:text-slate-500" />
                        </div>
                        <p class="text-sm font-semibold text-slate-700 dark:text-slate-300">No completed assessments</p>
